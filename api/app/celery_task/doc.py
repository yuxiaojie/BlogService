...
def gen_article(db_session):
    ...
    for k in all_article.keys():
        if k not in detected_article:
            print('miss article {}'.format(k))
            db_access.miss_article(k)
    db_session.commit()
    print('gen article complete')
...
def update_repo(home):
    ...
    if not os.path.exists(home):
        ...
        cmd = 'cd {} && {} git clone {}'.format(WORKING_DIR, GIT_SSH_COMMAND, REPOSITORY)
        print('not find repository on {}, try to clone ... '.format(home))
        print(cmd)
        p = os.popen(cmd)
        cmd_out = p.read()
        code = p.close()
        if code is None:
            print('clone repository success')
        else:
            print('clone repository failed, exit code: {}'.format(code))
            print('std out: {}'.format(cmd_out))
            return
    else:

        cmd = 'cd {} && {} git pull'.format(home, GIT_SSH_COMMAND)
        print('detected git home, try to pull ... ')
        print(cmd)
        p = os.popen(cmd)
        cmd_out = p.read()
        code = p.close()
        if code is None:
            print('pull repository success')
        else:
            print('pull repository failed, exit code: {}'.format(code))
            print('std out: {}'.format(cmd_out))
